...
export async function callLLM({ system, user, jsonMode = false, maxTokens = 1024, temperature = 0.3 }) {
  if (!isLLMAvailable()) {
...
- Vibrators: "第一次买的紧张感", "怕被室友听到", "值不值这个价"
- AI tools: "会不会替代我的工作", "API调用会不会突然涨价", "数据会不会被训练"
- Gaming: "这游戏值不值得全价", "会不会又是半成品", "新手选哪个职业"
...
Return JSON:
{
  "impulses": [
    {
      "id": 1,
      "impulse": "one short phrase describing the impulse (Chinese)",
      "impulse_en": "English version",
      "description": "2-3 sentences explaining what drives this impulse",
      "keywords": ["5 real search queries a person with this impulse would type"]
    }
  ]
}

Rules:
- Identify 10-20 distinct impulses from the discussions provided
- Each impulse must be genuinely different — not just rephrasing the same thing
- Keywords must be real queries people would actually type, not SEO-optimized phrases
- Think like the person searching, not a marketer
- Include impulses across the full emotional spectrum: anxiety, curiosity, desire, frustration, embarrassment, ambition
- Some impulses may only appear in 1-2 posts — that's fine, niche impulses are valuable
- Return JSON only, no explanation`,
    user: `Topic: ${topic}\n\nReddit discussions:\n${discussionText.slice(0, 8000)}`,
    jsonMode: true,
    maxTokens: 4096,
    temperature: 0.4, // Higher temp for creative impulse discovery
  });
