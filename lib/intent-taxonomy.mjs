...
export function rankPriority({ commercialScore = 0, semrushVolume = 0, liveSignalScore = 0, kd = 0, keyword = '' }) {
  ...
  // Estimate KD when not provided: Reddit signals + keyword generality as proxy
  let kdScore;
  if (kd > 0 && Number.isFinite(Number(kd))) {
    kdScore = Number(kd); // Use real data when available
  } else {
    // Estimate: more generic keywords = higher KD
    // "best vibrator" → generic → KD ~65
    // "quiet vibrator for apartment" → specific → KD ~25
    const words = (keyword || '').split(/\s+/).filter(Boolean).length;
    const hasModifier = /\b(quiet|waterproof|app[- ]?controlled|discreet|beginner|couple|small|mini|budget|under|travel|shower)\b/i.test(keyword || '');
    if (words <= 2 && !hasModifier) kdScore = 55 + Math.min(20, live * 0.2); // generic: "vibrator", "best vibrator"
    else if (words <= 3) kdScore = 40 + Math.min(15, live * 0.15); // moderate: "best vibrator for couples"
    else if (hasModifier) kdScore = 20 + Math.min(15, live * 0.1); // specific with modifier: "quiet vibrator for apartment"
    else kdScore = 30 + Math.min(20, live * 0.15); // long tail
  }
  ...
  // Hard cap: KD > 60 can never be P0
